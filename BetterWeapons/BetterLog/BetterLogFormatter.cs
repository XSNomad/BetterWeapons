...
namespace BetterWeapons
{
    public class BetterLogFormatter
    {
        ...
        {
            string line = string.Format(settings.LogLineFormat,
                GetFormattedTime(),
        ...
        public static readonly Regex NEWLINE_REGEX = new(@"\r\n|\n\r|\n|\r", RegexOptions.Compiled);
        ...
        {
            TimeSpan value = TimeSpan.FromSeconds(Time.realtimeSinceStartup);
        ...
        {
            string text = LogToString(logLevel);
            string formatted = string.Format(settings.LogLevelFormat, text);
            return formatted;
        }
        ...
        {
            string formatted = string.Format(settings.NameFormat, name);
            return formatted;
        }

        private string GetFormattedMessage(object message)
        {
            string formatted = string.Format(settings.MessageFormat, message);
            return formatted;
        }

        private string GetFormattedException(Exception exception)
        {
            string formatted = string.Format(settings.ExceptionFormat, exception);
            return formatted;
        }
        ...
        private string GetFormattedLocation(IStackTrace location)
        {
            ...
            StackTrace stackTrace = ((DiagnosticsStackTrace)location).stackTrace;
            if (stackTrace == null || stackTrace.FrameCount < 1)
            {
                return null;
            }
            System.Reflection.MethodBase method = stackTrace.GetFrame(0).GetMethod();
            string formatted = string.Format(settings.LocationFormat, method.DeclaringType, method.Name);
            return formatted;
        }
    }
}